...
class SQL:
    ...
    def table_existence_handler(self):
        ...
        if not self.cur.fetchone():
            if self.table == 'packets':
                query = f'CREATE TABLE {self.table} (serial MEDIUMINT, time DOUBLE, src_ip TEXT, src_port MEDIUMINT, dst_ip TEXT, dst_port MEDIUMINT, proto TINYTEXT, flag TINYTEXT, ttl MEDIUMINT,size MEDIUMINT, alert TINYINT);'
    ...
    def update_alert(self, serial:int):
        query = f'UPDATE {self.table} SET alert=1 WHERE serial={serial}'
        self.cur.execute(query)
        self.con.commit()
